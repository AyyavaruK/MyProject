...
$(document).ready(function(){
	getDetails('getCompensationActionDetails.htm');
});
function save(){
	if($('#mapdto').valid()){
		loading();
		...
		$.ajax({
			url:"saveCompensationAction.htm",
			type: "post",
...
function reset(){
	document.forms[0].action='compensationAction.htm';
...
function displayExistRecord(id){
	getDetailsById('getCompensationActionDetailsById.htm?id='+id);
}
</script>
