		...
		<table width="100%" border="0" cellspacing="1" cellpadding="5" bgcolor="E3E3E3" align="right">
	      <tr>
	        ...
	        <div class="innerpage-breadcrum">
				...
				<a href="javascript:void(0);" class="select">Compensation Action Reason</a>
				<span style="float: right">
					<a href="#" onclick="javascript:reset()" class="easyui-linkbutton" iconCls="icon-add">Add Compensation Action Reason</a>
	    ...
	    <table width="100%" border="0" cellspacing="1" cellpadding="5" class="data-table-new" align="right">
	      <tr>
		     <th colspan="6" style="text-align: left;padding-left: 10px" class="head">
    			Compensation Action Reason
    		 </th>
	      </tr>
	      <tr>
		     <td style="width:20%" nowrap="nowrap">
    			<label>Compensation Action Reason</label><span class="mandatory">*</span>
    		 </td>
	      ...
	      <tr>
	      	<td colspan="8">
	      		<table id="dataTable"  class="easyui-datagrid"  title="Compensation Action Reason Details Table"  style="width:100%;height:270px;table-layout: fixed;"
						...
						<thead>
							<tr>
								...
								<th data-options="field:'value',width:100" formatter="formatDetail">Compensation Action Reason</th>
...
$(document).ready(function(){
	getDetails('getCompensationActionReasonDetails.htm');
});
function save(){
	if($('#mapdto').valid()){
		loading();
		...
		$.ajax({
			url:"saveCompensationActionReason.htm",
			type: "post",
...
function reset(){
	document.forms[0].action='compensationActionReason.htm';
...
function displayExistRecord(id){
	getDetailsById('getCompensationActionReasonDetailsById.htm?id='+id);
}
</script>
